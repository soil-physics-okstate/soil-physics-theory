...
Ch = np.zeros(n+2, float)       
oldCh = np.zeros(n+2, float)      

def initialize(mySoil, psi0, T0):
    global z, dz, vol, psi, oldPsi, T, oldT, theta, oldTheta, Ch, oldCh
...
def solver(mySoil, myBoundary, isFreeDrainage, dt):
    global psi, oldPsi, T, oldT, theta, oldTheta, Ch, oldCh
    ...
    while (((massBalance > toleranceWater) or (energyBalance > toleranceHeat)) 
        ...
        dHydr[1] -= boundary.waterFlux(psi[1], theta[1], T[1], myBoundary, airResistance) *dt
        bHydr[1] -= (boundary.dWaterFluxdPsi(psi[1], theta[1], T[1], myBoundary, airResistance) *dt)
    ...
    if ((massBalance < toleranceWater) and (energyBalance < toleranceHeat)):
        isBalanceOk = True
    ...
    return (isBalanceOk, waterFlux, heatFlux, boundaryLayerConductance, evaporationFlux, nrIterations)
