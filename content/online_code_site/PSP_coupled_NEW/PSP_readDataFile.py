...
def scanDataFile(fileName, delimiter):
    reader = csv.reader(open(fileName, "rt"), delimiter=delimiter)
    nrRows = 0
    for row in reader:
        if (nrRows == 0): 
            nrCols = len(row) 
...
def readDataFile(fileName, nrHeaderFields, delimiter, isPrintScreen):
    nrRows, nrCols, isFileOk = scanDataFile(fileName, delimiter)
...
def readGenericDataFile(fileName, nrHeaderRows, delimiter, isPrintScreen):
    nrRows, nrCols, isFileOK = scanDataFile(fileName, delimiter)
    if (isPrintScreen): print ('nrRows =', nrRows, ' nrCols =', nrCols)
    
    if (isFileOK == False): return (nrRows, False)
    myReader = csv.reader(open(fileName, "rt"), delimiter=delimiter)
